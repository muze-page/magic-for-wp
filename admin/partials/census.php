...
function wporg_options_page() {
	add_menu_page(
		        ...
		        'manage_options',
		        'wporg',
		        'wporg_options_page_html',//要调用函数输出此页面的内容
	            'dashicons-chart-pie',
		        90
		    );
}
...
class Magick_Single_Census {
	...
	function magick_output_month_single( $arr, $id ) {
		for ( $i = 0; $i < count( (array)$arr ); $i++ ) {
			//拿到日期
			$time = $arr[ $i ];
			$args = array(
	...
	//输入一个日期，输出该日期的发文数量
	function magick_output_today_single( $arr ) {
		for ( $i = 0; $i < count( (array)$arr ); $i++ ) {
			//拿到日期
			$time = $arr[ $i ];
			$args = array(
